# Zero-Downtime Migration: Kustomize Configuration

This directory contains the Kubernetes manifests and Kustomize configurations for demonstrating zero-downtime migration techniques. The configuration is structured in three progressive phases using Kustomize overlays.

## Architecture

### Current

![Current Architecture](../arc-images/new-full-architecture.gif "Current Full Architecture")

### Future

![Future Architecture](../arc-images/future-full-architecture.gif "Future Full Architecture")
...
```bash
# Check pod status
kubectl get pods -n zerodt-demo

# View Envoy logs
kubectl logs -n zerodt-demo <pod-name> -c envoy

# Test Envoy admin interface
kubectl exec -n zerodt-demo <pod-name> -c envoy -- curl http://localhost:9901/ready

# Check applied configuration
kubectl get deployment -n zerodt-demo monolith-web-general-pool -o yaml
```
